...
class GPTSetting with _$GPTSetting {
  const factory GPTSetting(
      {@Default({}) Map<String, String> quickQuestion,
      @Default(false) bool useDirectSvc}) = _GPTSetting;
...
class GPTSettings extends _$GPTSettings {
  ...
  Future<(bool, String)> request(String question) async {
    ...
    final useDirectSvc = state.valueOrNull?.useDirectSvc ?? false;
    final url = useDirectSvc
        ? "/cyber/gpt/simple-question?question=$q&openai=$useDirectSvc"
        : "/cyber/gpt/simple-question?question=$q";
  ...
  Future<String> useDirectSvc(bool ok) async {
    final v = state.value;
    if (v == null) return "变更失败";
    final d = v.copyWith(useDirectSvc: ok);
    state = AsyncData(d);
